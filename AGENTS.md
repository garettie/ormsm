...
- Frameworks: React 19, Vite 7, TailwindCSS 4
- Libraries: Recharts, D3-Sankey, Supabase (PostgreSQL), Lucide React
- Package manager: npm

### Commands
- Install: `npm install`
- Dev (default): `npm run dev` (CallTree focus)
- Dev (RCSA): `npm run dev:rcsa` (sets `VITE_APP_MODE=rcsa`)
- Typecheck: `npm run typecheck` (`tsc --noEmit`)
- Lint: `npm run lint`
- Build: `npm run build`

### Architecture
- `src/components/`: Shared UI components (Badge, Login, AppShell)
- `src/lib/`: Shared utilities and Supabase client
- `src/modules/calltree/`: Incident management dashboard
- `src/modules/rcsa/`: Risk and Control Self-Assessment dashboard
- Supabase: Used for auth and data persistence. Client in `src/lib/supabase.ts`.

### Conventions
- Modularization: Keep module-specific logic inside `src/modules/[name]/`.
- Components: PascalCase. One component per file preferred.
- Environment: `VITE_APP_MODE` determines the initial active module in `App.tsx`.
- Data Fetching: Supabase client is used extensively in hooks.
